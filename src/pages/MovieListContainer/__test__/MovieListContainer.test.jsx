...
import { isLoading, itemsHasErrored } from '../../../modules/isLoading/isLoadingAction';
import clearCurrentMovie from '../../../modules/clearCurrentMovie/clearCurrentMovieAction';
import changeMainMovie from '../../../modules/changeMainMovie/changeMainMovieAction';
...
describe('MovieList renders correctly', () => {
  ...
  it('createGenreList renders correctly', () => {
...
describe('mainMovie Reducer', () => {
  ...
  it('mainMovie: request mainMovieDetails', () => {
    ...
    const id = 458156;
    fetchMock
      .getOnce(`https://api.themoviedb.org/3/movie/${id}?api_key=75331f1a740385460b25b56203149aa8language=en-US`, {
        headers: { 'content-type': 'application/json' },
        body: { page: 1, results: [1, 2, 3] },
        status: 200,
      });
    fetchMock.catch(err => itemsHasErrored(err));
    const store = mockStore({});
    store.dispatch(requestFilms.getMainMovieDetails(id)).then(dataMovie => expect(dataMovie).toEqual(action));
  });
});
...
describe('Requests tests', () => {
  ...
  it('Fetch popular movies', async () => {
    ...
    const store = mockStore({});
    // const test = new Promise(() => Promise.resolve(requestFilms.fetchPopular()))
    //   .then(resp => store.dispatch(resp))
    //   .then((data) => {
    //     expect(data).toEqual(expectedActions);
    //   });
  ...
  it('Fetch Top Rated', () => {
    ...
    const store = mockStore({});
    return store.dispatch(requestFilms.getTopRated(id)).then(data => expect(data).toEqual(expectedActions));
  });
...
//     genres: [{ id: 35, name: 'Drama' }, { id: 25, name: 'Action' }],
//   });
//   const store = mockStore({
//     genres: [{ id: 35, name: 'Drama' }],
//   });

//   // const genre = checkGenres(state);
//   checkGenres(store);
//   console.log('genres', checkGenres(state))
//   expect(checkGenres).toEqual([{ id: 35, name: 'Drama' }, { id: 25, name: 'Action' }]);
// });
